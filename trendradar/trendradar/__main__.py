...
from trendradar.opportunity_pipeline import OpportunityPipeline
...
class NewsAnalyzer:
    ...
    def _send_notification_if_needed(
        ...
        # 检查是否有有效内容（热榜或RSS）
        pipeline_mode = cfg.get("PIPELINE", {}).get("MODE", "legacy")
        if pipeline_mode == "opportunity":
            has_news_content = bool(current_results and any(bool(items) for items in current_results.values()))
            has_rss_content = bool(raw_rss_items and len(raw_rss_items) > 0)
        else:
        ...
        # 计算热榜匹配条数
        if pipeline_mode == "opportunity":
            news_count = sum(len(items or {}) for items in (current_results or {}).values())
            rss_count = len(raw_rss_items or [])
        else:
        ...
        if (
            ...
            pipeline_mode = cfg.get("PIPELINE", {}).get("MODE", "legacy")
            if pipeline_mode == "opportunity":
                print("[机会流程] 检测到 opportunity 模式，优先执行机会识别推送链路")
                ...
                pipeline_result = opportunity_pipeline.run(
                    stats=stats,
                    failed_ids=failed_ids,
                    new_titles=new_titles,
                    id_to_name=id_to_name,
                ...
                if not pipeline_result.get("fallback_to_legacy", False):
                    if pipeline_result.get("push_success", False):
                        if schedule.once_push and schedule.period_key:
                            scheduler = self.ctx.create_scheduler()
                            date_str = self.ctx.format_date()
                    ...
                    if pipeline_result.get("should_push", False):
                        print("[机会流程] 本轮满足机会推送条件，但没有渠道发送成功")
                    return False

                print("[机会流程] 新流程返回回退信号，继续执行 legacy 通知链路")
    ...
    def _execute_mode_strategy(
        ...
        time_info = self.ctx.format_time()
        pipeline_mode = self.ctx.config.get("PIPELINE", {}).get("MODE", "legacy")

        if pipeline_mode == "opportunity":
            print("[机会流程] opportunity 模式下跳过 legacy 关键词筛选、旧 AI 分析、翻译和旧 HTML 报告生成")
...
"""
    ...
    try:
        ...
        pipeline_mode = config.get("PIPELINE", {}).get("MODE", "legacy")
        if pipeline_mode == "opportunity":
            print("[流程] 检测到 pipeline.mode=opportunity")
            print("[流程] 本轮将优先使用原始热榜 + 原始 RSS 驱动机会识别流程")
