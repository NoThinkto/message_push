...
def _load_pipeline_config(config_data: Dict) -> Dict:
    """
    加载流程模式配置。

    这个配置用于在上层决定：
    - 是继续走旧版热点推送流程（legacy）
    - 还是尝试走新增的机会识别流程（opportunity）

    当前骨架版本默认仍然推荐 `legacy`，以确保现有系统稳定。
    """
    pipeline = config_data.get("pipeline", {})
    mode = (_get_env_str("PIPELINE_MODE") or pipeline.get("mode", "legacy")).strip().lower()
    if mode not in {"legacy", "opportunity"}:
        print(f"[流程] 未知的 pipeline.mode={mode}，自动回退为 legacy")
        mode = "legacy"
...
def _load_opportunity_pipeline_config(config_data: Dict) -> Dict:
    """
    加载“机会识别流程”配置。

    说明：
    1. 该配置段只服务于新增 opportunity 流程；
    2. 默认关闭，不影响旧流程；
    3. 字段保留较详细的中文语义，方便后续继续扩展。
    """
    cfg = config_data.get("opportunity_pipeline", {})
    channels = cfg.get("channels", {})

    enabled_env = _get_env_bool("OPPORTUNITY_PIPELINE_ENABLED")
    min_confidence_env = _get_env_str("OPPORTUNITY_PIPELINE_MIN_CONFIDENCE")
    on_ai_failure_env = _get_env_str("OPPORTUNITY_PIPELINE_ON_AI_FAILURE")

    try:
        min_confidence = (
            float(min_confidence_env)
            if min_confidence_env
            else float(cfg.get("min_confidence", 0.75))
        )
    except ValueError:
        min_confidence = 0.75
    ...
    return {
        ...
        "PROMPT_FILE": cfg.get("prompt_file", "opportunity_analysis_prompt.txt"),
        "TITLE_FILTER_PROMPT_FILE": cfg.get("title_filter_prompt_file", "opportunity_title_filter_prompt.txt"),
        "DEEP_ANALYSIS_PROMPT_FILE": cfg.get("deep_analysis_prompt_file", "opportunity_deep_analysis_prompt.txt"),
        "INTERESTS_FILE": cfg.get("interests_file", "opportunity_interests.txt"),
        "USE_RAW_CANDIDATES": cfg.get("use_raw_candidates", True),
        "DEDUPE_ENABLED": cfg.get("dedupe_enabled", True),
        "DROP_LOW_SIGNAL_TITLES": cfg.get("drop_low_signal_titles", True),
        "PER_SOURCE_LIMIT": cfg.get("per_source_limit", 6),
        "MAX_CANDIDATES": cfg.get("max_candidates", 60),
        "MAX_EVIDENCE_PER_CANDIDATE": cfg.get("max_evidence_per_candidate", 3),
        "MAX_PUSH_OPPORTUNITIES": cfg.get("max_push_opportunities", 3),
        "SECOND_PASS_ENABLED": cfg.get("second_pass_enabled", True),
        "SECOND_PASS_MAX_CANDIDATES": cfg.get("second_pass_max_candidates", 5),
        "SECOND_PASS_MIN_FETCH_PRIORITY": cfg.get("second_pass_min_fetch_priority", 0.45),
        "SECOND_PASS_REQUIRE_URL": cfg.get("second_pass_require_url", True),
        "ARTICLE_FETCH_TIMEOUT": cfg.get("article_fetch_timeout", 15),
        "ARTICLE_MAX_CHARS": cfg.get("article_max_chars", 6000),
        "MIN_CONFIDENCE": min_confidence,
        "ALLOWED_RISK_LEVELS": cfg.get("allowed_risk_levels", ["low", "medium"]),
        "REQUIRE_ACTIONABLE_TARGETS": cfg.get("require_actionable_targets", True),
        "REQUIRE_SUGGESTED_ACTION": cfg.get("require_suggested_action", True),
        "PUSH_WHEN_NO_OPPORTUNITY": cfg.get("push_when_no_opportunity", False),
        "ON_AI_FAILURE": on_ai_failure_env or cfg.get("on_ai_failure", "skip"),
        "MAX_NEWS_FOR_ANALYSIS": cfg.get("max_news_for_analysis", 80),
        "INCLUDE_RSS": cfg.get("include_rss", True),
        "ANALYSIS_DEDUPE_ENABLED": cfg.get("analysis_dedupe_enabled", True),
        "ANALYSIS_DEDUPE_TTL_HOURS": cfg.get("analysis_dedupe_ttl_hours", 24),
        "ANALYSIS_RETRY_FAILED": cfg.get("analysis_retry_failed", True),
        "ANALYSIS_FAILED_RETRY_AFTER_MINUTES": cfg.get("analysis_failed_retry_after_minutes", 60),
        "CHANNELS": {
            "WEWORK": channels.get("wework", True),
...
def _load_ai_config(config_data: Dict) -> Dict:
    ...
    thinking_enabled_env = _get_env_bool("AI_THINKING_ENABLED")
    ...
    return {
        ...
        "REASONING_EFFORT": _get_env_str("AI_REASONING_EFFORT") or ai_config.get("reasoning_effort", ""),
        "THINKING_ENABLED": thinking_enabled_env if thinking_enabled_env is not None else ai_config.get("thinking_enabled", None),
...
def load_config(config_path: Optional[str] = None) -> Dict[str, Any]:
    ...
    config["PIPELINE"] = _load_pipeline_config(config_data)
    config["OPPORTUNITY_PIPELINE"] = _load_opportunity_pipeline_config(config_data)
