...
class StorageManager:
    ...
    def get_recent_opportunity_analyzed_keys(
        self,
        item_keys,
        ttl_hours=24,
        retry_failed=True,
        failed_retry_after_minutes=60,
        date=None,
    ):
        """获取 opportunity 流程近期已深读分析过的候选 key。"""
        return self.get_backend().get_recent_opportunity_analyzed_keys(
            item_keys,
            ttl_hours=ttl_hours,
            retry_failed=retry_failed,
            failed_retry_after_minutes=failed_retry_after_minutes,
            date=date,
        )

    def save_opportunity_analyzed_item(
        self,
        item_key,
        title,
        url="",
        ...
        """记录 opportunity 流程已完成正文级分析的候选。"""
        return self.get_backend().save_opportunity_analyzed_item(
            item_key=item_key,
            title=title,
            url=url,
    ...
    def next_opportunity_push_sequence(self, date=None):
        """获取并递增当天 opportunity 推送序号。"""
        return self.get_backend().next_opportunity_push_sequence(date)
