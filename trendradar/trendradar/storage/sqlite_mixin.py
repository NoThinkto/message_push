...
class SQLiteStorageMixin:
    ...
    def _get_recent_opportunity_analyzed_keys_impl(
        self,
        item_keys: List[str],
        ttl_hours: int = 24,
        retry_failed: bool = True,
        failed_retry_after_minutes: int = 60,
        date: Optional[str] = None,
    ) -> set[str]:
        """获取 TTL 窗口内已经分析过、应跳过的 opportunity 候选 key。"""
        if not item_keys:
            return set()

        try:
            conn = self._get_connection(date, db_type="news")
            cursor = conn.cursor()
            now = self._get_configured_time().replace(tzinfo=None)
            success_cutoff = now - timedelta(hours=max(int(ttl_hours or 0), 0))
            failed_cutoff = now - timedelta(minutes=max(int(failed_retry_after_minutes or 0), 0))

            skip_keys: set[str] = set()
            chunk_size = 500
            for start in range(0, len(item_keys), chunk_size):
                chunk = item_keys[start:start + chunk_size]
                placeholders = ",".join("?" * len(chunk))
                cursor.execute(
                    f"""
                    SELECT item_key, last_status, last_analyzed_at
                    FROM opportunity_analyzed_items
                ...
                for row in cursor.fetchall():
                    ...
                    analyzed_at = self._parse_datetime(row[2])
                    if not analyzed_at:
                        continue

                    if status == "success" and analyzed_at >= success_cutoff:
                        skip_keys.add(item_key)
                    elif status != "success":
                        if not retry_failed or analyzed_at >= failed_cutoff:
                            skip_keys.add(item_key)

            return skip_keys
        except Exception as e:
            print(f"[存储] 查询 opportunity 已分析记录失败: {e}")
            return set()

    def _save_opportunity_analyzed_item_impl(
        self,
        item_key: str,
        title: str,
        url: str = "",
        ...
        """保存 opportunity 正文级分析记录，用于跨批次去重。"""
        if not item_key:
            return False
        ...
        try:
            ...
            cursor = conn.cursor()
            now_str = self._get_configured_time().strftime("%Y-%m-%d %H:%M:%S")
            source_names_json = json.dumps(source_names or [], ensure_ascii=False)
            cursor.execute(
                """
                INSERT INTO opportunity_analyzed_items
                    (item_key, title, url, source_type, source_names,
                     first_analyzed_at, last_analyzed_at, analyze_count,
                     last_status, last_error, created_at, updated_at)
                VALUES (?, ?, ?, ?, ?, ?, ?, 1, ?, ?, ?, ?)
                ON CONFLICT(item_key) DO UPDATE SET
                    title = excluded.title,
                    url = CASE WHEN excluded.url != '' THEN excluded.url ELSE opportunity_analyzed_items.url END,
                    source_type = excluded.source_type,
                    source_names = excluded.source_names,
                    last_analyzed_at = excluded.last_analyzed_at,
                    analyze_count = opportunity_analyzed_items.analyze_count + 1,
            ...
            print(f"[存储] 保存 opportunity 已分析记录失败: {e}")
            return False

    def _next_opportunity_push_sequence_impl(self, date: Optional[str] = None) -> int:
        """获取并递增当天 opportunity 推送序号。"""
        try:
            conn = self._get_connection(date, db_type="news")
            cursor = conn.cursor()
            push_date = self._format_date_folder(date)
            now_str = self._get_configured_time().strftime("%Y-%m-%d %H:%M:%S")
            cursor.execute(
                """
                INSERT INTO opportunity_push_records
                    (push_date, push_count, last_pushed_at, created_at, updated_at)
                VALUES (?, 1, ?, ?, ?)
                ON CONFLICT(push_date) DO UPDATE SET
                    push_count = opportunity_push_records.push_count + 1,
                    last_pushed_at = excluded.last_pushed_at,
                    updated_at = excluded.updated_at
                """,
                (push_date, now_str, now_str, now_str),
            )
            cursor.execute(
                "SELECT push_count FROM opportunity_push_records WHERE push_date = ?",
                (push_date,),
            )
            row = cursor.fetchone()
            conn.commit()
            ...
            print(f"[存储] 获取 opportunity 推送序号失败: {e}")
            return 0
