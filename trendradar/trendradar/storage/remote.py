...
class RemoteStorageBackend(SQLiteStorageMixin, StorageBackend):
    ...
    def get_recent_opportunity_analyzed_keys(
        ...
        return self._get_recent_opportunity_analyzed_keys_impl(
            item_keys,
            ttl_hours=ttl_hours,
    ...
    def save_opportunity_analyzed_item(
        ...
        success = self._save_opportunity_analyzed_item_impl(
            item_key=item_key,
            title=title,
            url=url,
    ...
    def next_opportunity_push_sequence(self, date=None):
        sequence = self._next_opportunity_push_sequence_impl(date)
        if sequence > 0:
            self._upload_sqlite(date)
        return sequence
